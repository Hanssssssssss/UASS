...
from models import tb_sepeda_motor as tb_sepeda_motorModel
...
class BaseMethod():
    ...
    @property
    def data(self):
        query = select(tb_sepeda_motorModel.sepeda_motor, tb_sepeda_motorModel.cc, tb_sepeda_motorModel.harga, tb_sepeda_motorModel.speed, tb_sepeda_motorModel.berat, tb_sepeda_motorModel.kapasitas_tangki_bensin)
        result = session.execute(query).fetchall()
        print(result)
        return [{'sepeda_motor': tb_sepeda_motor.sepeda_motor, 'cc': tb_sepeda_motor.cc, 'harga': tb_sepeda_motor.harga, 'speed': tb_sepeda_motor.speed, 'berat': tb_sepeda_motor.berat, 'kapasitas_tangki_bensin': tb_sepeda_motor.kapasitas_tangki_bensin} for tb_sepeda_motor in result]
...
class tb_sepeda_motor(Resource):
    ...
    def get(self):
        query = select(tb_sepeda_motorModel)
        data = [{'sepeda_motor': tb_sepeda_motor.sepeda_motor, 'cc': tb_sepeda_motor.cc, 'harga': tb_sepeda_motor.harga, 'speed': tb_sepeda_motor.speed, 'berat': tb_sepeda_motor.berat, 'kapasitas_tangki_bensin': tb_sepeda_motor.kapasitas_tangki_bensin} for tb_sepeda_motor in session.scalars(query)]
        return self.get_paginated_result('tb_sepeda_motor/', data, request.args), HTTPStatus.OK.value


api.add_resource(tb_sepeda_motor, '/tb_sepeda_motor')
